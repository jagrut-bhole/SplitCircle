...
export const getGroupUsersController = asyncHandler(async (req, res) => {
    try {
        const currentUserId = req.user?.id;
        if (!currentUserId) {
...
export const getGroupDetailController = asyncHandler(async (req, res) => {
    try {
...
export const addGroupExpenseController = asyncHandler(async (req, res) => {
    try {
        ...
        if (splitType === "EQUAL") {
            ...
            // Extract from splits
            allParticipantUsernames = splits.map((s) => s.username);
        }
...
export const updateGroupExpenseController = asyncHandler(async (req, res) => {
    try {
        const { groupId } = req.params;
        const { expenseId } = req.params;
        const currentUserId = req.user?.id;
        const { title, amount, paidByUsername, participantUsernames } = req.body;
...
export const deleteGroupExpenseController = asyncHandler(async (req, res) => {
    try {
        const currentUserId = req.user?.id;
        const { groupId } = req.params;
        const { expenseId } = req.params;
...
export const deleteGroupController = asyncHandler(async (req, res) => {
    try {
        const currentUserId = req.user?.id;
        const { groupId } = req.params;
