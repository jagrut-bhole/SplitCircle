...
export class ExpenseServices {
    ...
    async friendExpenseList(currentUserId, friendId) {
        const friendship = await this.checkFriendshipExists(currentUserId, friendId);
        if (!friendship) {
            throw new Error("Not friends with the user!!");
        }
    ...
    async updateFriendExpense(currentUserId, expenseId, updates) {
        ...
        const toChange = (updates.description && updates.description !== expense.note) ||
    ...
    // methods
    calculateSplits(friendId, currentUserId, amount, scenario) {
        let paidById;
        let currentUserOwes;
        let friendOwes;
        let splitType;
    ...
    async updateBalance(userId1, userId2, change, tx) {
