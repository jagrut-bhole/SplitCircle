...
export class EmailServices {
    ...
    async verifyConnection() {
        try {
            ...
            console.log("✅ Email service is ready (Gmail API OAuth2)");
        }
        catch (error) {
    ...
    async sendEmail(to, subject, html) {
        try {
            const info = await this.transporter.sendMail({
                from: `"SplitCircle" <${process.env.EMAIL_USER}>`,
                to: to,
                subject: subject,
                html: html,
            });
    ...
    `;
        return this.sendEmail(friendAddedEmail, `${addedByName} added you as a friend`, html);
    }
    async sendGroupInviteEmail(invitedUsername, invitedEmail, groupName, invitedByName, invitedByUsername) {
        const html = `
      <!DOCTYPE html>
      <html>
      <head>
    ...
    `;
        return this.sendEmail(invitedEmail, `You've been added to "${groupName}"`, html);
    }
    async sendSettlementEmail(recipient, settlement, paidBy) {
    ...
    `;
        return this.sendEmail(recipient.email, `${paidBy.name} recorded a payment of ₹${settlement.amount}`, html);
    }
}
