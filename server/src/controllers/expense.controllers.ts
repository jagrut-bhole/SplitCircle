...
export const addFriendExpenseController = asyncHandler(
  async (req: Request, res: Response) => {
    try {
      ...
      if (description.length === 0 || description.length > 50) {
        return res.status(400).json({
          message:
            "Description cannot be empty and must be under 50 characters!!",
          success: false,
        });
      }
      ...
      const friendId = (await userService.extractCurrentUserId(
        username,
      )) as string;
...
export const friendExpenseListController = asyncHandler(
  async (req: Request, res: Response) => {
    try {
      ...
      const result = await expenseServices.friendExpenseList(
        currentUserId,
        friendId,
      );
...
export const singleExpenseDetailController = asyncHandler(
  async (req: Request, res: Response) => {
    try {
      ...
      const result = await expenseServices.friendSingleExpenseDetail(
        expenseId,
        currentUserId,
      );
...
export const updateFriendExpenseController = asyncHandler(
  async (req: Request, res: Response) => {
    try {
...
export const deleteFriendExpenseController = asyncHandler(
  async (req: Request, res: Response) => {
    try {
      const { expenseId } = req.params;
