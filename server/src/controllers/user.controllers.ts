...
export const userSearchControler = asyncHandler(
  async (req: Request, res: Response) => {
    try {
      ...
      const result = await userService.searchUserByUsername(
        friendUsername,
        currentUserId,
      );
...
export const addFriendController = asyncHandler(
  async (req: Request, res: Response) => {
    try {
      ...
      if (
        error.message === "Both are already friends" ||
...
export const getUserTotalOwedController = asyncHandler(
  async (req: Request, res: Response) => {
    try {
      ...
      return res
        ...
        .json({
          message:
            "Failed to fetch the user's owed data. Please try again later!!",
          success: false,
        })
        .status(500);
    }
...
export const getUserTotalGroupsController = asyncHandler(
  async (req: Request, res: Response) => {
    try {
      const userId = req.user?.id as string;
      ...
      return res
        ...
        .json({
          message:
            "Failed to fetch the user's groups. Please try again later!!",
          success: false,
        })
        .status(500);
    }
...
export const getFriendController = asyncHandler(
  async (req: Request, res: Response) => {
    try {
