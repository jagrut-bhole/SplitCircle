...
export class UserService {
  ...
  async getUserIdUsernames(usernames: string[]): Promise<Map<string, string>> {
    ...
    const notFound = usernames.filter(
      (username) => !usernameToIdMap.has(username),
    );
  ...
  async searchUserByUsername(friendUsername: string, currentUserId: string) {
    const user = await prisma.user.findUnique({
      where: {
  ...
  async addFriend(currentUserId: string, friendUserId: string) {
    ...
    const checkResult = await this.checkFriendshipExists(
      currentUserId,
      friendUserId,
    );
      ...
      for (const expense of expenses) {
        ...
        if (currentUserSplit && friendSplit) {
          const currentUserPaid =
            expense.paidById === currentUserId ? expense.amount : 0;
