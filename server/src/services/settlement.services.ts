...
export class SettlementService {
  async getFriendSettlementInfo(currentUserId: string, friendId: string) {
  ...
  async settleFriendSettlement(
    ...
    if (amount > actualBalance) {
      throw new Error(
        `You can't settle more than you owe. You owe ₹${actualBalance.toFixed(2)}, but trying to pay ₹${amount.toFixed(2)}`,
      );
    }
  ...
  async groupSettlementInfo(currentUserId: string, groupId: string) {
    ...
    for (const member of group.members) {
      if (member.userId !== currentUserId) {
        ...
        if (currentUserBalance < 0 && otherBalance > 0) {
          const proportionalDebt = Math.min(
    ...
    yourDebts.sort((a, b) => {
      if (a.youOwe && b.youOwe) return b.youOwe - a.youOwe;
      if (a.youOwe) return -1;
      if (b.youOwe) return 1;
      return 0;
    });
  ...
  async settleGroupDebt(
    ...
    if (amount > debtToUser.youOwe) {
      throw new Error(
        `You can't settle more than you owe. You owe ₹${debtToUser.youOwe.toFixed(2)}, but trying to pay ₹${amount.toFixed(2)}`,
      );
    }
