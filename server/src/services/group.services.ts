    ...
    async addGroupExpense(
        ...
        for (const participantId of participantIds) {
            const isGroupMember = group.members.some(m => m.userId === participantId);
                if (!isGroupMember) {
                    const username = participantUsernames.find(un => 
                        usernameToIdMap.get(un) === participantId
                    );
                throw new Error(`User "${username}" is not a member of this group`);
            }
        }

        const nonParticipantMembers = group.members.filter(member => 
