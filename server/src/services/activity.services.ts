    ...
    try {
      ...
      const formattedActivities = activities.map((activity: any) => {
        ...
        if (activity.settlement) {
          const paidByName =
            activity.settlement.paidBy.id === userId
              ? "You"
              : activity.settlement.paidBy.name;
          const paidToName =
            activity.settlement.paidTo.id === userId
              ? "You"
        ...
        // Handle expense activities
          ...
          const paidByName =
            activity.expense.paidBy.id === userId
              ? "You"
          ...
          if (activity.expense.splitType === "SETTLEMENT") {
            ...
            if (recipient) {
              const recipientName =
                recipient.userId === userId ? "You" : recipient.user.name;
              formattedNote = `${paidByName} paid ${recipientName}: ₹${activity.expense.amount.toFixed(2)}`;
            }
          } else {
            // Regular expense
            formattedNote = activity.note.replace(
              activity.expense.paidBy.name,
              paidByName,
            );
          }
        ...
        // Handle other activities - replace actor name with "You" if it's the current user
          ...
          const activityActor: any = activity.actor;
          if (
            activityActor &&
